...
Partial Public Class frmMain
            ...
            If cmdBack = LHSerialPort.cmdDistribute Then  '应该分340继续和正常启动两种情况
                ...
                If cmd = LHSerialPort.cmdStartup Then
                    ...
                    Else                                '1000启动
                        With _unit(unitNo)
                            .Testing = &H0
                            .lastHour = Now.Date.AddHours(Now.Hour - 1)
                        End With
    ...
    Private Sub IntegralTask()
        ...
        For k = 0 To 31
            ...
            _Xelconn = New OleDb.OleDbConnection("Provider=Microsoft.Ace.OleDb.12.0;Data Source=" _
                                & _unit(k).结果文件 & ";Extended Properties='Excel 8.0'")
            _Xelconn.Open()
            Do While Date.Compare(_unit(k).lastHour, Now.Date.AddHours(Now.Hour)) < 0
                '超过58分暂停召回

                _unit(k).lastHour = _unit(k).lastHour.AddHours(1)
                If Date.Compare(_unit(k).lastHour.Date, lastdate.Date) = 0 Then
                    i = _unit(k).lastHour.Hour
                    If ((lastrecord(i \ 8) >> (i Mod 8)) And 1) Then '数据有效，记录
                        IntegralPerHour(k)
                        DBMethord.UpdateHour(k, _unit(k).lastHour)
                    End If
                End If
                If Date.Compare(_unit(k).lastHour.Date, thisdate.Date) = 0 Then
                    i = _unit(k).lastHour.Hour
                    If ((thisrecord(i \ 8) >> (i Mod 8)) And 1) Then '数据有效，记录
                        IntegralPerHour(k)
                        DBMethord.UpdateHour(k, _unit(k).lastHour)
                    End If
                End If
